 ...
 * Done by Сидорак Данило (group Статистика 2курс/1підгрупа)
 * Date 25.05.2022
 *
...
public:
    ...
    void resize(const size_t &t_size) {
        if (t_size < 1) throw std::logic_error("Can`t resize with negative size");
        m_coefficients.resize(t_size);
        m_size = t_size;
    }
     ...
     *
     * it is defined as: f(x)^-1,
     * using f^-1 * f = 1 mod x^(deg(f))
     *
     ...
     *
     * uses idea Rev(f) = Rev(g)Rev(q) + x^(deg(f)-deg(r))Rev(r)
     * Rev(f) = Rev(g)Rev(q) mod x^deg(q)
     * Rev(q) = Rev(f)(Rev(g))^-1 mod x^deg(q)
     *
     * @param - IN g(x)
     * @result - new ComplexPolyP that contains operation result
     */
    ComplexPolyP operator/(const ComplexPolyP &poly) const {
        ...
        // supposed we have Rev(f) as coefficients going backwards from a_N-1 to a_0
        // res = Rev(f) * Rev(g)^-1 mod 2(N-2)
        ComplexPolyP s = tmp2.recip();
        ComplexPolyP res = (tmp1 * s).scale(-2 * n);

        // if deg(f) twice as big deg g then deg(q) > deg(g)
        if(m > 2*n) {
            // Rev(tmp3) = 1
